...
```{r canopy_frag, echo=FALSE}
knitr::opts_knit$set(root.dir = "E:/FragEVI/")
read.csv("processed")
cum.dist.forest <- as.data.table(fread("processed/bos.can.cummdist.forest.csv"))
cum.dist.dev <- as.data.table(fread("processed/bos.can.cummdist.dev.csv"))
cum.dist.hdres <- as.data.table(fread("processed/bos.can.cummdist.hdres.csv"))
cum.tot <- as.data.table(fread("processed/boston/bos.lulc_candist.csv"))
can.area.total <- cum.tot[!is.na(bos.lulc.lumped), sum(bos_can01_filt, na.rm=T)]/1E4 ## 3941 ha canopy total
can.edge.area.total <- cum.tot[!is.na(bos.lulc.lumped), round(sum(bos.ed10, na.rm=T))]/1E4 ## 3187 ha of edge canopy
...
```
## Canopy configuration in Boston
Tree canopy in the study area was extremely fragmented, with `r round(can.edge.area.total/can.area.total, 2)*100` percent of total canopy area within 10 m of an edge (Figure 1). Canopy distribution and fragmentation degree differed somewhat with LULC type. Developed and High-density Residential areas covering XX and Y % respectively of the total urban area contained AA and BB %, repspectively, of total canopy area, of which CC and DD % was within 10 m of an edge. In contrast, Forest areas covered only GG % of the study area but contained FF % of the total urban canopy, of which a lower fraction (RR %) was within 10 m of an edge.
