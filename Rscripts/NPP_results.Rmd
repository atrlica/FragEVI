...
andy <- as.data.table(read.csv("processed/andy.bai.ps.dbhincr.csv"))
...
load("processed/mod.fia.final.sav") ## called yyy
load("processed/mod.andy.final.sav") ## called g.full
load("processed/mod.street.final.sav") ## called mod.street.dbhdelta.me
...
Estimated biomass gain (MgC/yr) in the combined urban forest + street tree (Hybrid) model was `r as.character(round(npp.dat[aoi>800 & !is.na(lulc), sum(hyb.npp.mod.lin, na.rm=T)/2000],0))` MgC, with the largest annual biomass gains accruing in the Forest and HD Residential land use types (*Table 1*). In contrast, estimates of biomass gain made only on the basis of urban forest growth rates (including edge effects) predicted somewhat lower total biomass gain of `r as.character(round(npp.dat[aoi>800 & !is.na(lulc), sum(andy.npp.tot.mod.lin, na.rm=T)/2000], 0))` MgC. The Hybrid model showed greater biomass accural primarily to non-forested land use types such as High-Density Residential, partly a result of accounting for biomass growth in these open-canopy areas by simulating collections of stems growing at higher street tree-like rates. By comparison, an estimation approach that applied rural forest growth factors to per-ha-canopy biomass density showed lower biomass gain in all land use categories (total gain of `r round(npp.dat[aoi>800 & !is.na(lulc), sum(fia.empir.npp.kg.hw.forest, na.rm=T)/2000], 0)`) MgC, with a greater relative fraction of total biomass gain accruing to forest stands. This reduced estimate, particuarly in non-forested cover types, is partly the result of lower per-stem and per-area biomass gain in rural forest context than seen in urban forest or street trees. The Hybrid estimate apportioned biomass gains similarly to the distribution of total biomass across the study area, with most biomass gains occuring in the more common moderate-density pixels dominated by the open-canopy High-Density Residential type (*Figure 3*). The rural forest model, in contrast, applied lower rural forest-like growth factors overall and apportioned relatively more total biomass gain into the less common high-density pixels dominated by Forest land cover type. In the Hybrid model, median per-pixel annual biomass gain was `r round(npp.dat[aoi>800 & !is.na(lulc), median(((hyb.npp.mod.lin/2000)/aoi)*1E4, na.rm=T)], 2)` MgC/ha compared with `r round(npp.dat[aoi>800 & !is.na(lulc), median(((fia.empir.npp.kg.hw.forest/2000)/aoi)*1E4, na.rm=T)], 2)` MgC/ha in the rural forest model, with the largest comparative increases in Forest, HD Residential and LD Residential areas. 
...
The model results illustrate the sensitivity of estimating biomass productivity via its relationship to biomass density due to the ambiguity of the areal basis to be used in determning biomass density in heterogeneous scattered urban tree stands. An example of typical discontinous urban canopy in the study area shows that at moderate levels of both canopy and impervious cover, estimates of biomass density in a given area can vary from 25 MgC/ha-ground to 76 MgC/ha-canopy to 404 MgC/ha-pervious (*Figure 4*). In the same area, common remotely sensed measures of vegetation cover and function such as NDVI may indicate a relatively barren condition though the area contains appreciable canopy. Across the Boston study area, the method of determining the areal basis (ground, canopy, or pervious area) for biomass density across the study area had significant and differential effects on biomass density frequency distributions that varied with land use type (*Figure S2*). On a canopy basis, biomass density tended somewhat higher in Forest type pixels dominated by larger mature trees, while developed pixel types showed a wider lower range of densities, possibly reflecting a wider mix of generally younger street and backyard trees. In contrast, biomass density distributions on a ground-area basis were skewed lower due to the high frequency of discontinuous canopy pixels. Density distributions on a pervious-area basis showed a higher portion of both extremely high and extremely low values. As this study used the empirical relationship between stand biomass density and productivity on an areal basis measured in closed-canopy forest to estimate biomass gain, ambiguities in the driving biomass density will result in ambiguities in estimated productivity. For instance, in comparison to the results obtained via canopy-area biomass density estimates, applying rural forest growth factors to biomass density on a ground-area basis rendered an estimate of `r round(npp.dat[aoi>800, sum(andy.npp.tot.mod.lin, na.rm=T)/2000], 0)`, reasonably similar to the estimate obtained using edge-specific urban forest growth factors (*Table 1*). These coinciding results, rather than reflecting similar underlying ecosystem processes, are instead likely an artifact of the neagatively shifted distribution of pixel biomass density when judged on a ground-area basis due to the prevalence of discontinuous tree stands in the study area. 
...
```{r fig_S2, echo=FALSE, out.width='100%'}
knitr::include_graphics('E:/FragEVI/images/FigS2_biom_dens_methods_byLULC.png')
```
