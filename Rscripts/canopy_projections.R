...
for(o in hitlist){
  ...
  for(pix in 1:length(dbh.sav)){ ## the number of pixels in this sim chunk
    if(length(cage.biom.sim[[pix]])>=40){ ## only process if enough simulations successfully completed
      ...
      for(a in 1:100){   ### resimulate dbh history in every pixel 100 times
        ...
        npp.lims <- quantile(cage.ann.npp[[pix]], probs=c(0.40, 0.6)) ## restrict which of the simulations to draw and modify
        ...
        # j <- sample(1:100, 1) ## or just sample the whole simulation collection at random
        ...
        for(y in 1:length(tree.samp)){ ## simulate each tree history separately
          d <- tree.samp[y] ## a single dbh.2006 record in this pixel tree collection
          # track <- d
          for(e in 1:36){ ## test each tree for 36 years (2006-2040)
            deathwatch <- mort(d) ## recalculate death probability given size
            if(rbinom(1, 1, deathwatch/100)){ ## if tree doesn't make it
              ...
              # print("It's alive!!")
              d <- d*(1+dbhg.pred(d)) ## update size via growth if it lives
            }
            # track <- c(track, d)
            # ptrack <- c(ptrack, deathwatch)
          } ## loop for 36 year projector
          dbh.track <- c(dbh.track, d) ## store up the new dbhs after growth and morts applied
          # plot(track)
          # track <- numeric()
        ...
        deaths.track <- c(deaths.track, deaths)
      ...
      deaths.sav[[pix]] <- deaths.track ## just a vector n=100 with number of deaths recorded in each separate simulation per pixel (how much replanting is needed in every pixel over 40 years)
 ...
 save(dbh.sav, file = paste("processed/boston/biom_street/dbh.resim.scenario-0.BAU.", o, ".V1.sav", sep=""))
 save(deaths.track, file=paste("processed/boston/biom_street/death.track.scenario-0.BAU", o, ".V1.sav", sep=""))
}
